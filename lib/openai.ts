...
export async function generateDescription(prompt: string) {
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: `Generate a product description for an Amazon listing. ${prompt}`,
      },
    ],
  });
...
export async function generateImage(prompt: string) {
  ...
  const response = await client.images.generate({
    prompt: `Generate for me an image for the following product: ${prompt}`,
    n: 1,
    size: "512x512",
  });
  ...
  //   return `https://via.placeholder.com/512x512.png?text=${encodeURIComponent(
  //     prompt
  //   )}`;
  // }
  // try {
